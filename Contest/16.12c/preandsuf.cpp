...
namespace sub3{
    ...
    void solve(){
        ...
        FOR(j, 1, __lg(n)) FOR(i, 1, n-MASK(j)+1) F0R(ch, M) st[i][j][ch] = max(st[i][j-1][ch], st[i+MASK(j-1)][j-1][ch]);
